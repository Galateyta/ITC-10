...
void MainWindow::addImageElemets(QObject *view, QObject *parent, EElementType parentType, QDomElement e, QString style, QString src)
{
    if (parentType == EElementType::Div)
    {
        ...
        if(rx.cap(0).length() != 0)
         ...
         {
             download->getImage(src, label);


//              std::string data = download->getImage(src);
    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(imageDownloader, &DownloadManager::imagefinished, this, &MainWindow::onImageDownloadFinished);
...
void MainWindow::onImageDownloadFinished(void* usrPtr, QByteArray data)
{
    QLabel* label = static_cast<QLabel*>(usrPtr);
    QPixmap pix;
    pix.loadFromData(data);
